...
class _FlutterEarthState extends State<FlutterEarth>
  ...
  List<Offset> clipTiles(Rect clipRect, double radius) {
    final list = <Offset>[];
    final scale = math.pow(2.0, zoomLevel) as double;
    final observed = HashMap<int, int>();
    ///////////////////////////////////////
    final lastKeys = List.filled(clipRect.width ~/ 10 + 1, 0, growable: true);
  ...
  void initMeshTexture(Mesh mesh) {
    final tile =
        getTile(mesh.x! ~/ tileWidth, mesh.y! ~/ tileHeight, zoomLevel);
    if (tile!.status == TileStatus.ready) {
      //Is zoomed tile?
      if (tile.z != zoomLevel) {
  ...
  Mesh initMeshFaces(Mesh mesh, int subdivisionsX, int subdivisionsY) {
    int? faceCount = subdivisionsX * subdivisionsY * 2;
    final List<Polygon> faces = <Polygon>[];
    ...
    mesh.indexCount = mesh.indexCount! + faceCount;
  ...
  Mesh buildPoleMesh(
    ...
    mesh.vertexCount = mesh.vertexCount! + vertexCount;
    mesh.x = -1;
    mesh.y = -1;
    mesh.texture = image;
  ...
  void drawTiles(Canvas canvas, Size size) {
    ...
    meshList.sort((Mesh a, Mesh b) {
      return b.z!.compareTo(a.z as num);
    });
  ...
  void initState() {
    ...
    loadImageFromAsset('asset/google_map_north_pole.png')
        .then((Image value) => northPoleImage = value);
    loadImageFromAsset('asset/google_map_south_pole.png')
